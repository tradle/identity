...
function Identity(props) {
  ...
  props = props || {};
  var keys = props[KEYS_KEY];
  this._props = extend(true, {}, props);

  this._keys = [];
  if (keys) {
    delete this._props[KEYS_KEY];
    props[KEYS_KEY].forEach(this.addKey, this);
  }
}
...
Identity.fromJSON = function(json) {
  ...
  json[KEYS_KEY] = keys.filter(function(key, idx) {
    try {
      return toKey(key);
    } catch (err) {
  ...
  Object.keys(json).forEach(function(p) {
    if (p in PROP_TO_CL) {
      var Cl = PROP_TO_CL[p];
      json[p] = json[p].map(function(props, idx) {
        debugger;
